...
module ad_serdes_in #(
  ...
  generate if (FPGA_TECHNOLOGY == SEVEN_SERIES) begin
    for (l_inst = 0; l_inst <= (DATA_WIDTH-1); l_inst = l_inst + 1) begin: g_data
      ...
      i_idelay (
        ...
        .LD (up_dld[l_inst]),
        .CNTVALUEIN (up_dwdata[((DRP_WIDTH*l_inst)+DRP_WIDTH-1):(DRP_WIDTH*l_inst)]),
        .CNTVALUEOUT (up_drdata[((DRP_WIDTH*l_inst)+DRP_WIDTH-1):(DRP_WIDTH*l_inst)]));
  ...
  generate if (FPGA_TECHNOLOGY == ULTRASCALE || FPGA_TECHNOLOGY == ULTRASCALE_PLUS) begin
    ...
    i_idelay(
       ...
       .CNTVALUEOUT (up_drdata[((DRP_WIDTH*l_inst)+DRP_WIDTH-1):(DRP_WIDTH*l_inst)]), // 9-bit output: Counter value output
